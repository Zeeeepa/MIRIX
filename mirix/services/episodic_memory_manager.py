...
class EpisodicMemoryManager:
    ...
    def create_episodic_memory(
        ...
        # Create the episodic memory item (with Redis JSON caching)
        with self.session_maker() as session:
            episodic_memory_item = EpisodicEvent(**episodic_memory_dict)
            episodic_memory_item.create_with_redis(session, actor=actor)  # ⭐ Caches to Redis JSON
    ...
    def list_episodic_memory(
        ...
        # ⭐ NEW: Try Redis Search first (if Redis is enabled)
        from mirix.database.redis_client import get_redis_client
        redis_client = get_redis_client()
