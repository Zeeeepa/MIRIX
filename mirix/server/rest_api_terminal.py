...
# Middleware to resolve X-Client-Id into X-Org-Id for all routes
@app.middleware("http")
async def inject_client_org_headers(request: Request, call_next):
    """
    Resolve client/org headers for all endpoints.
    """
    public_paths = {
        "/terminal/clients",
        "/admin/auth/register",
    ...
    if x_client_id:
        try:
            client_id, org_id = get_client_and_org(x_client_id=x_client_id)
        ...
        return JSONResponse(
            status_code=401,
            content={"detail": "X-Client-Id header is required for this endpoint"},
        )
...
def get_client_and_org(
    ...
    if not x_client_id:
        raise HTTPException(
            status_code=401,
            detail="X-Client-Id header is required for this endpoint",
        )

    from mirix.orm.errors import NoResultFound

    try:
        client = server.client_manager.get_client_by_id(x_client_id)
...
def _get_admin_user_id_for_client(client_id: str) -> str:
    """
    Get the admin user ID for a client.
    """
    server = get_server()
    admin_user = server.user_manager.get_admin_user_for_client(client_id)
    if not admin_user:
        raise HTTPException(
            status_code=404,
            detail=f"Admin user not found for client {client_id}",
        )
...
# ============================================================================
# Terminal Client Bootstrap Endpoint
# ============================================================================


class TerminalClientCreateRequest(BaseModel):
    """Request model for creating a terminal client."""

    client_id: Optional[str] = Field(None, description="Optional client ID to create.")
    client_name: Optional[str] = Field(None, description="Optional client name.")


@router.post("/terminal/clients")
async def create_terminal_client(request: TerminalClientCreateRequest):
    """
    Create a client for terminal access and return the client_id.

    This endpoint is public and intended for environments without the dashboard.
    """
    if not request.client_id and not request.client_name:
        raise HTTPException(
            status_code=400,
            detail="Provide at least one of client_id or client_name",
        )
    ...
    if request.client_id:
        try:
            existing_client = server.client_manager.get_client_by_id(request.client_id)
            return {
    ...
    name = request.client_name or request.client_id or "Terminal Client"

    client = server.client_manager.create_client(
        Client(
            id=request.client_id if request.client_id else None,
            name=name,
            status="active",
            scope="read_write",
    ...
    admin_user = _create_terminal_admin_user(client)
...
def _create_terminal_admin_user(client: Client) -> User:
    """
    Create a client-scoped admin user for terminal access.
    """
    server = get_server()

    from mirix.services.user_manager import UserManager

    existing_admin = server.user_manager.get_admin_user_for_client(client.id)
    if existing_admin:
        raise HTTPException(
            status_code=409,
            detail=f"Admin user already exists for client {client.id}",
        )
    ...
    admin_user = server.user_manager.create_user(
        User(
            id=admin_user_id,
    ...
    # Create default memory blocks for the admin user (best-effort).
    try:
        from mirix.constants import CORE_MEMORY_BLOCK_CHAR_LIMIT

        server.block_manager.create_or_update_block(
            block=Block(
                label="human",
                value="",
        ...
        logger.warning(
            "Failed to create default blocks for terminal admin user %s: %s",
            admin_user.id if admin_user else "unknown",
            exc,
        )
...
async def add_memory(
    ...
    if not user_id:
        ...
        user_id = _get_admin_user_id_for_client(client.id)
...
def get_client_from_jwt_or_api_key(
    ...
    """
    Authenticate using client ID header (terminal access).
    
    Args:
        authorization: Unused (terminal access does not accept JWT tokens)
        request: FastAPI request to inspect injected headers (from X-Client-Id middleware)
        
    Returns:
        tuple: (client, auth_type) where auth_type is "client_id"
    ...
    raise HTTPException(
        status_code=401,
        detail="Authentication required. Provide X-Client-Id header.",
    )
    ...
    try:
        ...
        return TokenResponse(
            ...
            client=DashboardClientResponse(
                ...
                admin_user_id=_get_admin_user_id_for_client(client.id),
...
async def list_dashboard_clients(
    ...
    return [
        DashboardClientResponse(
            ...
            scope=c.scope,
            status=c.status,
            admin_user_id=_get_admin_user_id_for_client(c.id),
