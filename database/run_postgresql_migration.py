...
def run_migration(db_uri, sql_file_path):
    ...
    print(
        f"Connecting to database: {conn_params['database']} on {conn_params['host']}:{conn_params['port']}"
    )
...
def main():
    ...
    if not db_uri:
        ...
        print("Or:")
        print(
            "  python run_postgresql_migration.py 'postgresql+pg8000://user@host:port/database'"
        )
        sys.exit(1)

    # SQL file path
    sql_file_path = os.path.join(
        os.path.dirname(__file__), "migrate_database_postgresql.sql"
    )
