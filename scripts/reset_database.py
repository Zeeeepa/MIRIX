...
def reset_postgresql(
    ...
    if drop_result.returncode == 0:
        ...
        log_warning(
            f"Database drop had issues: {drop_result.stderr.strip() if drop_result.stderr else 'Unknown error'}"
        )
    ...
    if create_result.returncode == 0:
        ...
        if "already exists" in create_result.stderr:
            ...
            log_error(
                f"Database creation failed: {create_result.stderr.strip() if create_result.stderr else 'Unknown error'}"
            )
