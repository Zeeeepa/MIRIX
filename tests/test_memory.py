...
def test_search_methods(agent):
    ...
    test_tracker.start_test(
        "Search Methods Test",
        "Testing different search methods across all memory types",
    )
    ...
    try:
        ...
        subtest_idx = test_tracker.start_subtest(
            "Testing Episodic Memory Search Methods"
        )
        try:
            ...
            test_tracker.pass_subtest(
                subtest_idx, "All episodic search methods completed"
            )
        except Exception as e:
        ...
        subtest_idx = test_tracker.start_subtest(
            "Testing Procedural Memory Search Methods"
        )
        try:
            for method in search_methods:
                for field in search_fields["procedural"]:
                    try:
                        ...
                        raise e
            test_tracker.pass_subtest(
                subtest_idx, "All procedural search methods completed"
            )
        except Exception as e:
            import ipdb
        ...
        subtest_idx = test_tracker.start_subtest(
            "Testing Resource Memory Search Methods"
        )
        try:
            for method in search_methods:
                for field in search_fields["resource"]:
                    ...
                    if method == "embedding" and field == "content":
                        print(
                            f"  {method} on {field}: Skipped (no embeddings for this field)"
                        )
                        continue
                    try:
                        ...
                        raise e
            test_tracker.pass_subtest(
                subtest_idx, "All resource search methods completed"
            )
        except Exception as e:
            test_tracker.fail_subtest(e, subtest_idx)
        ...
        subtest_idx = test_tracker.start_subtest(
            "Testing Knowledge Vault Search Methods"
        )
        try:
            for method in search_methods:
                for field in search_fields["knowledge_vault"]:
                    ...
                    try:
                        ...
                        raise e
            test_tracker.pass_subtest(
                subtest_idx, "All knowledge vault search methods completed"
            )
        except Exception as e:
            test_tracker.fail_subtest(e, subtest_idx)
        ...
        subtest_idx = test_tracker.start_subtest(
            "Testing Semantic Memory Search Methods"
        )
        try:
            for method in search_methods:
                for field in search_fields["semantic"]:
                    try:
                        ...
                        raise e
            test_tracker.pass_subtest(
                subtest_idx, "All semantic search methods completed"
            )
        except Exception as e:
            test_tracker.fail_subtest(e, subtest_idx)
...
def test_fts5_comprehensive(agent):
    ...
    memory_types = [
        (
            "episodic",
            agent.client.server.episodic_memory_manager.list_episodic_memory,
            agent.agent_states.episodic_memory_agent_state,
            ["summary", "details"],
        ),
        (
            "procedural",
            agent.client.server.procedural_memory_manager.list_procedures,
            agent.agent_states.procedural_memory_agent_state,
            ["summary", "steps"],
        ),
        (
            "resource",
            agent.client.server.resource_memory_manager.list_resources,
            agent.agent_states.resource_memory_agent_state,
            ["title", "content"],
        ),
        (
            "knowledge_vault",
            agent.client.server.knowledge_vault_manager.list_knowledge,
            agent.agent_states.knowledge_vault_agent_state,
            ["caption", "secret_value"],
        ),
        (
            "semantic",
            agent.client.server.semantic_memory_manager.list_semantic_items,
...
def test_fts5_performance_comparison(agent):
    ...
    for query_type, query in test_queries:
        ...
        if results_summary.get("bm25", {}).get("success") and results_summary.get(
            "string_match", {}
        ).get("success"):
...
def list_all_memory_content(agent):
    ...
    try:
        ...
        for i, event in enumerate(episodic_memory[:5]):  # Show first 5
            print(
                f"  {i + 1}. [{event.event_type}] {event.summary} (Actor: {event.actor})"
            )
    ...
    try:
        ...
        for i, kv in enumerate(knowledge_items[:5]):  # Show first 5
            print(
                f"  {i + 1}. [{kv.entry_type}] {kv.caption} (Sensitivity: {kv.sensitivity})"
            )
...
def test_specific_memory_search(
    agent, memory_type, query, search_method="bm25", search_field=None, limit=10
):
    """
    ...
    try:
        if memory_type == "episodic":
            ...
            for i, kv in enumerate(results):
                ...
                print(
                    f"      Secret Value: {kv.secret_value[:20]}..."
                    if len(kv.secret_value) > 20
                    else f"      Secret Value: {kv.secret_value}"
                )
...
def test_all_indirect_memory_operations(agent):
    ...
    test_tracker.start_test(
        ...
        "Testing message-based interactions with memory agents",
    )
...
def test_all_search_and_performance_operations(agent):
    ...
    test_tracker.start_test(
        ...
        "Testing different search methods and performance comparisons",
    )
    ...
    try:
        ...
        test_tracker.pass_test(
            "All search and performance tests completed successfully"
        )
...
def test_greeting():
    ...
    response = agent.send_message(message="Hello again!", memorizing=False)
    print(response)
...
def test_greeting_with_images():
    ...
    agent.client.server.cloud_file_mapping_manager.add_mapping(
        file_ref.uri, "img_1.jpg", datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    )
    message = [
        {"type": "text", "text": "Show me what is in this image"},
...
def test_greeting_with_files(file_path):
    ...
    print("\n--- Test 1: OpenAI-style file format ---")
    message = [
        {"type": "file_uri", "file_uri": file_path},
        {
            "type": "text",
...
def run_file_tests():
    ...
    test_tracker.start_test(
        "File Handling Tests",
        "Testing file uploading and processing with multiple AI providers",
    )
    ...
    try:
        ...
        if not os.path.exists(file_path):
            test_tracker.fail_test(
                f"Test file {file_path} not found. Please ensure it exists in the current directory."
            )
            return
...
def test_procedural_memory_direct(agent):
    ...
    # Cleanup
    agent.client.server.procedural_memory_manager.delete_procedure_by_id(
        procedure_id=procedure.id, actor=agent.client.user
    )
...
def test_resource_memory_direct(agent):
    ...
    # Cleanup
    agent.client.server.resource_memory_manager.delete_resource_by_id(
        resource_id=resource.id, actor=agent.client.user
    )
...
def test_knowledge_vault_direct(agent):
    ...
    # Cleanup
    agent.client.server.knowledge_vault_manager.delete_knowledge_by_id(
        knowledge_vault_item_id=knowledge.id, actor=agent.client.user
    )
...
def test_semantic_memory_direct(agent):
    ...
    # Cleanup
    agent.client.server.semantic_memory_manager.delete_semantic_item_by_id(
        semantic_memory_id=semantic_item.id, actor=agent.client.user
    )
...
def test_resource_memory_update_direct(agent):
    ...
    try:
        agent.client.server.resource_memory_manager.delete_resource_by_id(
            resource_id=resource1.id, actor=agent.client.user
        )
        agent.client.server.resource_memory_manager.delete_resource_by_id(
            resource_id=resource2.id, actor=agent.client.user
        )
...
def test_tree_path_functionality_direct(agent):
    ...
    print(
        f"Inserted episodic event with tree path: {' > '.join(episodic_event.tree_path)}"
    )
    ...
    procedure = agent.client.server.procedural_memory_manager.insert_procedure(
        ...
        steps=["Test step 1", "Test step 2", "Complete"],
        actor=agent.client.user,
    ...
    print(
        f"Inserted semantic item with tree path: {' > '.join(semantic_item.tree_path)}"
    )
    ...
    for result in episodic_results:
        if hasattr(result, "tree_path") and result.tree_path:
            print(
                f"Episodic: {result.summary} -> Tree Path: {' > '.join(result.tree_path)}"
            )
    ...
    try:
        agent.client.server.episodic_memory_manager.delete_event_by_id(
            episodic_event.id, actor=agent.client.user
        )
        agent.client.server.procedural_memory_manager.delete_procedure_by_id(
            procedure.id, actor=agent.client.user
        )
        agent.client.server.resource_memory_manager.delete_resource_by_id(
            resource.id, actor=agent.client.user
        )
        agent.client.server.semantic_memory_manager.delete_semantic_item_by_id(
            semantic_item.id, actor=agent.client.user
        )
...
def test_core_memory_update_using_meta_memory_manager(agent):
    ...
    response, agent_type = agent.message_queue.send_message_in_queue(
        ...
        kwargs={"message": test_message, "message_queue": agent.message_queue},
...
def test_episodic_memory_indirect(agent):
    ...
    test_message3 = (
        "Update the last episodic memory, because I didn't buy chicken two days ago."
    )
...
def test_resource_memory_update_indirect(agent):
    ...
    print(
        ...
        [
            res.id
            for res in agent.client.server.resource_memory_manager.list_resources(
                agent_state=agent.agent_states.resource_memory_agent_state,
                actor=agent.client.user,
                limit=100,
            )
